...
export default function Terminal() {
  ...
  const run = (raw: string) => {
    ...
    if (cmd === "memories") {
      setLines((prev) => [
        ...prev,
        { type: "output", text: "Iniciando processo de memória..." },
        { type: "output", text: "" }
      ]);
    ...
    const responseLines = output ?? UNKNOWN;

    setLines((prev) => [
      ...prev,
      ...responseLines.map((text) => ({ type: "output" as const, text })),
  ...
  return (
    <section id="terminal-section" className="relative py-28 px-6 bg-deep">
      ...
      >
        ...
        >
          {lines.map((line, i) => (
            <div key={i} className={`leading-relaxed ${line.type === "input" ? "text-teamgreen" : "text-[#CCCCCC]"}`}>
              {line.type === "input" && (
                <span className="text-teampurple mr-2">C:\MEMORIES&gt;</span>
              )}
              {line.text || "\u00A0"}
            </div>
          ))}
          ...
          <div className="flex items-center mt-1">
            <span className="text-teampurple font-mono text-sm mr-2">C:\MEMORIES&gt;</span>
            <input
              ref={inputRef}
              type="text"
              value={value}
              ...
              className="flex-1 bg-transparent outline-none text-teamgreen font-mono text-sm caret-teamgreen"
              autoComplete="off"
              spellCheck={false}
            />
          </div>
