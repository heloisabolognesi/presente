...
export default function StarField({ density = 90 }: { density?: number }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < density; i++) {
      ...
      // Color variation using team colors
      const rng = Math.random();
      if (rng > 0.90) {
        star.style.background = "#22C55E"; // green
      } else if (rng > 0.80) {
        star.style.background = "#8B5CF6"; // purple
      } else {
    ...
    // Occasional shooting stars (tech style)
